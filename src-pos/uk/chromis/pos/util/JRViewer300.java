...
public final class JRViewer300 extends javax.swing.JPanel implements JRHyperlinkListener
{
    ...
    private void initTouchLayout() {
        Dimension buttonSize = TouchUI.reportToolbarButtonSize();
        setToolbarButtonSize(btnSave, buttonSize);
        setToolbarButtonSize(btnPrint, buttonSize);
        setToolbarButtonSize(btnReload, buttonSize);
        setToolbarButtonSize(btnActualSize, buttonSize);
        setToolbarButtonSize(btnFitPage, buttonSize);
        setToolbarButtonSize(btnFitWidth, buttonSize);
        setToolbarButtonSize(btnZoomIn, buttonSize);
        setToolbarButtonSize(btnZoomOut, buttonSize);
        setToolbarButtonSize(btnFirst, buttonSize);
        setToolbarButtonSize(btnPrevious, buttonSize);
        setToolbarButtonSize(btnNext, buttonSize);
        setToolbarButtonSize(btnLast, buttonSize);

        Dimension zoomSize = new Dimension(96, TouchUI.REPORT_FILTER_FIELD_HEIGHT);
        cmbZoom.setMinimumSize(zoomSize);
        cmbZoom.setMaximumSize(zoomSize);
        cmbZoom.setPreferredSize(zoomSize);

        Dimension pageSize = TouchUI.reportFilterShortFieldSize();
        txtGoTo.setMinimumSize(pageSize);
        txtGoTo.setMaximumSize(pageSize);
        txtGoTo.setPreferredSize(pageSize);

        scrollPane.getHorizontalScrollBar().setUnitIncrement(TouchUI.SCROLLBAR_WIDTH);
        scrollPane.getVerticalScrollBar().setUnitIncrement(TouchUI.SCROLLBAR_WIDTH);
    }

    private void setToolbarButtonSize(JButton button, Dimension size) {
        button.setMinimumSize(size);
        button.setMaximumSize(size);
        button.setPreferredSize(size);
    }

    private void setToolbarButtonSize(JToggleButton button, Dimension size) {
        button.setMinimumSize(size);
        button.setMaximumSize(size);
        button.setPreferredSize(size);
    }
